...
1. 変更を加え、`npm test`と`npm run lint`が通ることを確かめます。テストの走らせ方は[README](README.md)の「テストとカバレッジ」にあります
...
- `npm run lint`はMarkdownの文書を`markdownlint`で検査し、日本語の文書は`textlint`でも検査します。スクリプトは検査の対象外です
