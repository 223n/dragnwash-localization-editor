...
- (g) 単独の CR。値の中の単独の CR（後ろに LF の続かない CR）と、引用符で囲まない値が引用の外の単独の CR で切れた形の2つ。前者は訳の入ったレコードだけを止める。上流の道具はそのレコードをあとで落とすので、LF に直すよう案内する（決まったことのそのほか 1）。訳の空のレコードはどちらの道具でも公開されないので止めない（原文は翻訳者には直せない）。訳の入ったレコードの原文（source_en 列）で止めたときは、LF に直す案内を出さない。キーは原文から作るので、原文を直すとキーと合わず、その行は R15 で捨てられ、malformed dropped に数えられるだけで止まりも知らせもしない。上流の hash-strings.ps1 も、原文に単独の CR のある行を公開しない（pwsh 7.6.6 で、架空の作業コピーを通しで走らせて確かめた）。そのため、原文は直さず、その行の訳を空に戻せばほかの行は書けると案内する。key 列の単独の CR は、取り除くよう案内する（LF に直すと、キーの途中の改行でキーの形でなくなることがある）。後者は csvfile.FindCRCuts で見つけ、訳の有無によらず止める（決まったことの 11 と 14）。ゲームは引用の外の CR を捨ててつなげて読むが、publish は切れた前半だけを書き、後半の訳は黙って落ちるためである。
