...
書き込みの錠（PR3。改善の決定 3）。画面の保存は、版の照合から rename までを OS の錠（Windows は LockFileEx、Linux・macOS などは flock）で囲む。錠は、利用者のキャッシュのフォルダーの dwloc/locks に、書き出し先の実体の絶対パスから名前を作って置くファイルに掛け、消さずに残す（publish.LockFile）。書き出し先の横に置いて放すときに消す形は、翻訳リポジトリに錠のファイルが残りうることと、Windows で消したファイルを開けない・2つが同時に錠を持つことがあったので採らなかった。dwloc publish は、組み立てに使った入力のバイト列を覚え、書く直前に入力へ同じ錠を掛けてから読み直し、違えば1バイトも書かずに止める（終了コード1）。上流の道具には無い振る舞いで、上流とのバイト一致は変えない（止めないときの出力は同じ）。
