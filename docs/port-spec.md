...
- (f) 飲み込み（引用符が別の行で閉じ、後ろの行を値に飲み込んだと見られる形）。見分けは csvfile.FindSwallows（下の「全体を解釈する読み手（csvfile）」）で、行をまたぐレコードの続きの物理行が、単独で読むとレコードに見える（キーの形で始まる、区切りの数がヘッダーと同じ）か、行をまたいだ引用が閉じたすぐ後ろに文字が続くと止める。飲み込まれたと疑う物理行ごとに1件を出す。ただし、閉じ引用符の後ろに文字が続く行があるレコードは、その行だけを publish_swallow_text_after_quote の理由で出す（続きの行がレコードに見えても、その理由では出さない）。入力・いまの公開ファイル・入力と書き出し先が同じファイルのどの経路でも止める（決まったことの 4）。同じファイルの中の訳を同じファイルのいまの訳と比べても、食い違いは見えないためである。直し方は、引用符の閉じ位置を確かめ、値を閉じる " が抜けていれば足し、値の中の " は "" と書くこと。
- (g) 単独の CR。値の中の単独の CR（後ろに LF の続かない CR）と、引用符で囲まない値が引用の外の単独の CR で切れた形の2つ。前者は訳の入ったレコードだけを止める。上流の道具はそのレコードをあとで落とすので、LF に直すよう案内する（決まったことのそのほか 1）。訳の空のレコードはどちらの道具でも公開されないので止めない。訳の入ったレコードの原文（source_en 列）で止めたときは、原文を直してよいかがその行のキーの決まり方（R12〜R17。publish.LoneCRKeyKind が rowKey と同じ規則で決め、理由の置換 key_kind に入れる）で変わるので、直し方を分ける。列名だけで「原文は直さない」と案内すると、台詞ID の行でも公開できる訳を捨てさせるためである（検証の指摘）。台詞ID の行はキーを原文から作らない（R12）ので、LF に直すか取り除くよう案内する。key 列のキーがいまの原文から作ったものと一致する行は、原文を直すとキーと合わず、その行は R15 で捨てられ、malformed dropped に数えられるだけで止まりも知らせもしないので、原文は直さず、その行の訳を空に戻せばほかの行は書けると案内する。key 列が無いか空の行は、原文を直すとキーが変わり（R14）、ゲームが引かないキーで公開されるので、同じく訳を空に戻すよう案内する。改行（CRLF と単独の CR）を LF にそろえると key 列のキーと一致する行は、いまは R15 で捨てられている（キーは Mod が LF の原文から計算する。エディターなどが改行を CR に変えると起きる）ので、LF にそろえるよう案内する（取り除くとキーと合わない）。どちらでもキーと合わない行は、直しても公開されないと案内する。上流の hash-strings.ps1 は、原文に単独の CR のある行を正しいキーで公開しない（7列の作業コピーでは書かず、2列の作業コピーでは CR の後ろの残り（閉じ引用符を含む）から作ったキーで書く）。原文を LF にそろえたとき・CR を取り除いたときは、どのキーの決まり方でも dwloc と同じキーで書く（書かない）。ただし2列の作業コピーで原文を LF にそろえると、続きの行がヘッダーと同じ列の数に見えるので、dwloc は飲み込みの確かめで止まり、そのレコードの --accept-multiline <ロケール>:<key> を付けると同じキーで書く（pwsh 7.6.6 で、架空の作業コピーを通しで走らせて確かめた。cmd/dwloc の TestPublishLoneCRSourceFix が、同じ形の入力で dwloc の結果と案内の文面を固定する）。キーの決まり方（key 列のキー、台詞ID、2列）ごとの代表は、上流との突き合わせの入力の表にも置き、上流の結果を pwsh 7.4.6 で作った正解で固定する（lone-cr-source-key、lone-cr-source-line-id、lone-cr-source-2col。決まったことの 19。下の「上流と意図して違える点」）。key 列の単独の CR は、取り除くよう案内する（LF に直すと、キーの途中の改行でキーの形でなくなることがある）。後者は csvfile.FindCRCuts で見つけ、訳の有無によらず止める（決まったことの 11 と 14）。ゲームは引用の外の CR を捨ててつなげて読むが、publish は切れた前半だけを書き、後半の訳は黙って落ちるためである。
- (h) 再生順のデータ（data/script_order.csv と data/level_flow.csv）の値のうち、publish が引用せずにそのまま書く値の改行（CR か LF）。見出しの行（'# ===== … =====' と '# --- … ---'）へ書く script_order.csv の section・phase・node・condition と level_flow.csv の dragon・weather・set_flags・end_flags、本体の行へエスケープせずに書く order（R24f）を見る（決まったことのそのほか 8。publish.CheckOrderShape）。全体を解釈して読むと引用した値に改行が入りうる。そのまま書くと見出しの2行目が '#' で始まらない行になり、次に publish で読むときデータの行として読まれ、その行に '"' があれば後ろの行を飲み込む（上流も同じ壊れ方をする）。再生順のデータの閉じない引用符も同じ関数が (e) として見る。どちらもどのロケールにも効くので、ロケールを数えあげる前（publish）、再生順を読む前（画面の書き出し）に見る。確かめたうえで通す指定では通さない。diff は見出しの文言を使わないので、level_flow.csv の閉じない引用符では判定を止めず、標準エラーに1行の警告だけを出す（決まったことの 18。script_order.csv の閉じない引用符は報告全体を止める）。

飲み込みの確かめは、正しい複数行の値でも当たることがある（原文の2行目がカンマを多く含む ml-continuation-looks-like-row、source_en,translation の2列の作業コピーで原文が行をまたぐ ml-source-translated-2col）。そのため、続きの行がレコードに見える形だけは、確かめたうえで通す指定（dwloc publish --accept-multiline <ロケール>:<key>。--path で走らせたときは <--path に渡したファイル>:<key>）で、レコードごとに通せる（決まったことの 16）。key は飲み込んだレコードの key（publish.RecordKey。key 列の値の前後の空白を除いたもの、key 列が無いか空なら原文から作るキー）で、台詞ID は綴りのまま、それ以外は ASCII の大文字小文字を区別せずに比べる（publish.SameKey。R13 と同じ）。止めたときの直し方に、写せる形の指定を出す。key を publish.Visible の印に置き換えると写しても当たらないので、そのまま出す。そのため、指定に使えない文字（ASCII の英数字と . _ : - のほか）を含む key と空の key（飲み込んだのがヘッダーのとき、key 列も原文も空のレコード）は、指定を案内せず、指定でも通さない（publish.NameableKey）。どちらのレコードも publish は書かない（R15〜R17）。同じファイルに同じ key のレコードが2つ以上あるときも、どのレコードを確かめたのかを指定から決められないので通さず、要らないレコードを消すよう案内する（publish が書くのは訳の入った最初のレコードだけ。R18）。ゲームの作業コピー（実物の ja で重複0件）と publish の書く公開ファイルでは key が重ならないので、これは手で直したファイルだけに起きる。指定は、そのロケールの入力・いまの公開ファイル・ゲーム側の公開ファイルのどれでも、その key のレコードに効き、ほかのロケールのゲーム側の公開ファイルには効かない（cmd/dwloc の TestPublishAcceptMultiline「ゲーム側の公開ファイルにも効き、ほかのロケールの指定では通さない」）。通した行と、通した指定は標準エラーに出す。直し方の指定と通した行に添える指定は、値（ロケール名か --path のファイルのパスと key）に ASCII の英数字と `.` `_` `-` `:` `/` のほかの文字があれば、二重引用符で囲んで出す（cmd/dwloc の shellWord）。Steam の既定の場所（Program Files (x86)）やゲームのフォルダー名（Drag'n Wash）のように空白・括弧・`'` のあるパスを囲まずに出すと、貼ったときにシェルが語に割り、使い方の誤り（終了コード2）になるためである（検証の指摘）。二重引用符は PowerShell・cmd・bash のどれでも、これらの文字をそのまま渡す。バックスラッシュも囲む（bash は引用符の外のバックスラッシュを取り除く）。二重引用符の中でも読み替えるシェルのある文字（bash と PowerShell の `$` と `` ` ``、bash の `!` と続けて書いた `\\`、cmd の `%`、PowerShell が引用符として読む `“` `”` `„`、引用符そのもの、制御文字）があるときは、囲んだ形のまま、使うシェルに合わせて書き直すよう添える。ロケールだけ（--path ではファイルだけ）の指定は受けず、レコード単位で指定するよう案内して終了コード2で止める。当たらない指定（無いロケール、--path に無いファイル、そのレコードに通せる行が無い）も終了コード2で止める。レコードに当たらないときは、形の報告を先に出す（当たらない理由が、そのレコードの形にあることがあるため）。閉じ引用符の後ろに文字が続く形は、どの書き手（上流の Escape-Csv、ゲームの WorkingCopy.cs が使う CsvReader.Escape）も作らないので通さない。続きの行が単独で読むとレコードに見えても、同じレコードにこの形があれば通さない（swallow-7col-empty-key-english と swallow-6col-published の3行目は、区切りの数がヘッダーと同じだが、閉じ引用符の後ろに文字が続く。通すと英語の原文やキーが訳として公開される。TestPublishAcceptMultilineKeepsOtherStops は、そのレコードを名指す指定でも書かないことを確かめる）。閉じない引用符と単独の CR も、直せる形なので通さない。画面の書き出しにはこの指定が無く、止まったときの文面で dwloc publish の指定を案内する。正しい複数行の値を一度公開すると、いまの公開ファイルの形の確かめで毎回当たるので、そのレコードを書くたびに同じ指定が要る。ロケール単位で通していたあいだ（決まったことの 4 の例）は、その指定が同じロケールに新しく入った飲み込みまで通していた。レコード単位では、新しく入った飲み込みは別のレコードなので止まる（cmd/dwloc の TestPublishAcceptMultiline「同じロケールのほかの飲み込みは通さない」）。ただし、同じレコードの値に新しく入った飲み込みは、同じ指定で通る。通した行を毎回標準エラーに出すのは、このためでもある。画面の書き出しは、そのロケールでは止まり続ける。
...
ゲーム側の土台の確かめ（CheckBase）は、コミット済みとゲーム側の訳を、値の中の CRLF を LF にそろえてから比べる（決まったことのそのほか 1。台詞ID の行も同じ）。翻訳リポジトリの .gitattributes は `*.csv text eol=lf` なので、コミットで値の中の CRLF も LF になる一方、ゲーム側の公開ファイルは書いたままの CRLF のことがあり、改行コードだけの違いで「ゲームが古い」と止めないためである。単独の CR はそろえない（git も変えず、上流の道具はその行を落とす）。publish が書く値の改行は、読んだとおりのままである（上流とバイト一致）。
...
全体を解釈する読み手へ移す作業（PR0〜PR4）の PR1 で、internal/csvfile に読み方の土台を足した。PR2 で publish（集め方・失われる訳の確かめ・土台の確かめ・形の確かめ）、diff（ReadRows と ParseLayoutRisks）、order（LoadPowerShell）がこの読み手へ移った（行だけが要る使い手は PowerShellFile.Rows を使う）。edit は PR2 で行の種類（見出し・空行・ヘッダー・データ行）を区切りの関数で決めるようにし、PR3 でセグメントを行にして、レコードの単位で保存するようにした（下の「使い手の扱い」）。行単位の読み手（csvfile.ReadPowerShellRows・ReadPowerShellRowsNumbered・ReadPowerShellTable）と、PR1 まで publish の守りが使っていた csvfile.ReadPowerShellWhole は、どの使い手も使わなくなり、回帰試験と上流との突き合わせの表のためにだけ残す。ここに書くのは、その土台の約束である。

区切りの関数（csvfile.SplitSegments）は、ファイル全体を1つの文字列として、コメント行・空行・レコードのセグメントに分ける。読み方の規則はこの関数だけが持ち、主の読み手、守り専用の読み手（csvfile.ReadPowerShellWhole）、形の検出が、みなこの結果に載る。保存（internal/edit）も PR3 からここに載り、レコードの最終フィールドの開始位置から本体の終わりまでを差し替える。書く側と読む側で区切りを別々に持つと、最終フィールドの位置がずれて、訳ではない値を書き換えるためである。
...
- 最初のレコードがヘッダーになる。"," や '""' の行でもヘッダーになる。ヘッダーより後ろの、読むとフィールドが0個か空の1個になるレコード（"," や '""' の行）は、空のレコードとして種類を分け、データとしては落とす。2列の作業コピーでキーの空いた行の訳を消すと、その行は空のレコードに変わる。保存（PR3）は、原文が空で key 列が16桁のキーでも台詞ID でもないレコード（key 列も原文も空のレコードを含む）をはじめから編集させない（下の edit_no_key_or_source。決まったことの 24）ので、訳を消してこの形になることは無い（PR3 のはじめは断らずに空行相当にしていた）。保存は、読むと値がどれも空になるレコード（",,,,,," の行など）も空行相当として編集させない（改善の ui-15）。
...
形の検出は、読み手が見分けない引用符の閉じ誤りなどを、形を見て見つける。止めるかどうかは呼び出し側が決め、publish と画面の書き出し（PR2 から。形の確かめ (f)(g)(h)）と画面の編集（PR3 から）は同じ関数を呼ぶ。物理行を単独で読むときは1物理行の読み方（csvfile.ParsePowerShellRecord と csvfile.FieldOffsets の中身）を使う。

- 飲み込み（csvfile.FindSwallows）: 行をまたぐレコードの続きの物理行を単独で読み、最初の値がキーの形（前後の空白を除き、台詞ID か、小文字にして16桁の16進）か、区切りの数（引用の外のカンマの数+1）がヘッダーの列数と同じなら、レコードに見えるとして返す。空行・空白だけの行・'#' の行は見ない。閉じない引用符のレコードも見ない。飲み込まれた行が自分の値を引用符で開く形（`one,"いち` の次に `"Alpha line` が来るなど）では、その開き引用符が前の行の閉じ忘れを閉じ、後ろの文字は閉じ引用符の後ろの文字として訳に足される。その行を単独で読むと引用が開いたまま終わるので、上の2つに当たらない。そこで、行をまたいだ引用が閉じたすぐ後ろに文字が続く物理行も返す（理由は text-after-quote）。レコードにこの行が1つでもあれば、そのレコードはこの行だけを返す（同じ行がキーの形や区切りの数に当たっても、理由は text-after-quote にする）。正当な値ではありえないレコードを、確かめたうえで通す指定で通せる理由で返さないためである。閉じ引用符の後ろに文字を書く書き手は無い（上流の Escape-Csv も、ゲームの WorkingCopy.cs が使う CsvReader.Escape も値全体を引用し、後ろは区切りか改行）ので、値が改行で終わる形を含め、正当な複数行の値では当たらない。こちらは全体を解釈したときの閉じ方で見るので、'#' の行でも当たる。同じ行の中で開いて閉じた引用の後ろの文字は見ない。上流との突き合わせの入力の表では、飲み込みの8件がすべて当たり（自分の値を引用符で開く3件と、区切りの数にも当たる swallow-7col-empty-key-english・swallow-6col-published の計5件は text-after-quote で当たる）、実物と同じ形の複数行の原文や複数行の訳は当たらない。正当でも当たるものが2件ある（原文の2行目がカンマを多く含む ml-continuation-looks-like-row と、2列の作業コピーで原文が行をまたぐ ml-source-translated-2col）。これらは確かめたうえで通す指定（dwloc publish --accept-multiline <ロケール>:<key>）で、レコードごとに書く。原文に単独の CR のある2列の作業コピー（lone-cr-source-2col）も、単独の CR で物理行が分かれ、続きの行がヘッダーと同じ2列に見えるので、同じ理由で当たる。こちらは単独の CR の確かめ (g) でも止まるので、指定しても書かない。
...
- 値の中の単独の CR（csvfile.LoneCRValues）と、見出しに使う値の中の改行（csvfile.LineBreakValues。publish の形の確かめ (h) が再生順のデータに使う）。
...
使い手の扱い（PR2）。閉じない引用符は、どの使い手も読み手の型付きの誤り（または ReadPowerShellMarked の印）で知り、それぞれ次のように扱う（決まったことの 3）。

- publish と画面の書き出し: 形の確かめ (e) で止める（上の「上流の変更への守り」）。再生順のデータ（data/script_order.csv と data/level_flow.csv）も、読む前に同じく (e) と (h) で止める。
- diff: そのファイルの行を1つも使わず、そのファイルに依るカテゴリだけを「判定していません（作業コピーの N行目の引用符が閉じません）」にして報告を続ける。公開ファイルならそのロケールのすべてのカテゴリと、ほかのロケールのロケールどうしを比べるカテゴリ（他のロケールにあって無い行、どのロケールにも訳が無い行）、作業コピーなら作業コピーを要るカテゴリ、layout_risks.csv ならはみ出しの恐れ、data/script_order.csv なら報告全体を止める。level_flow.csv は見出しの文言を使わないので止めず、終了コードも変えない。ただし publish と画面の書き出しは形の確かめで止まるので、dwloc diff はどの行で開いたかを標準エラーに1行の警告として書く（決まったことの 18。diff.Repo.LevelFlowUnclosed）。判定していないカテゴリの行は出さない。dwloc diff はどのファイルの何行目かを標準エラーへ書き、終了コードを1にする（決まったことのそのほか 6）。画面も起動時の diff が止まらないので開き、断り書きで同じことを言う。
- edit（画面の保存）: ファイル全体を読み取り専用にし（reason edit_unclosed_quote）、引用符が開いたレコードから後ろを、レコードとして解釈しない物理行のまま並べる。ヘッダーの受理は生テキストの完全一致のまま残し、どのセグメントをヘッダーにするかだけを区切りの関数にそろえた（決まったことのそのほか 8。ヘッダーの前の "," の行はヘッダーになって受理されず、全角空白だけの行は空行になる）。

edit の保存（PR3）。行はセグメントで、ID（セグメントの通し番号）と最初と最後の物理行を持つ。保存と画面の要求は ID とキーで行を指し、物理行の番号は表示と報告の照合にだけ使う（決まったことの 2）。保存はレコードの最終フィールドの開始位置から本体の終わりまでを差し替え、元の終端を保つので、原文が行をまたぐレコードも書ける（PR3 では訳が1行に収まるかぎり。訳の改行は下の PR4）。書く前の事後確認を2段で行う（決まったことのそのほか 4）。差し替えたレコードだけを区切りの関数とゲームの読み方で読み直し、外れたらその行を断る。書く直前にファイル全体を1回読み直し、セグメントの数・ID・物理行の範囲・触っていない行のバイト列が変わらず、書き換えたレコードが形の検出に当たらないこと、書き換えたレコードのほかはゲームの読み方で読んだ値が保存の前後で変わらないこと（キーか原文を鍵に比べる。キーも原文も空のレコードの訳を書き換えると、ゲームの引用の閉じる位置が動き、後ろのレコードをゲームが見つけられなくなることがあったため。そのレコードはいまは編集させないので、判定をすり抜けた書き換えを捕まえる守りになる）を確かめ、外れたら1バイトも書かずに行を特定できる誤りを返す。形の確かめは、正しく組み立てても外れることがある（値が改行で終わる列を持つレコードに、カンマで始まる訳を書くと、続きの物理行を単独で読んだときヘッダーと同じ列の数に見え、飲み込みの疑いに当たる）。書き換えたレコードが2つ以上なら、1つずつ（ほかの書き換えを戻したバイト列で）確かめ直し、それだけでも外れる最初のレコードを指す（外れたところから決めると、キーも原文も空のレコードとその後ろのレコードを1回で書き換えたとき、単独なら書ける後ろのレコードを指して画面がその送り直しを止めた。PR3 の検証の指摘）。どれも単独では外れないときは、外れたところに最も近い書き換えたレコードを指す。次のレコードは理由を付けて編集させない。

- 飲み込みの疑い（csvfile.FindSwallows。edit_swallow）。publish の形の確かめ (f) と同じ見分け方で、画面には確かめたうえで通す指定を置かない。
- ゲームの読み方との食い違い（csvfile.CSharpDisagreements。edit_game_disagrees、edit_game_misses_record）。全角空白や NO-BREAK SPACE だけの行がヘッダーの前にあるファイルでは、ゲームはその行をヘッダーにするので、どのレコードもこれに当たる。
- publish がキーを決められない（edit_no_key_or_source。決まったことの 24）。原文（source_en。トリムしない）が空で、key 列（前後の空白を除く）が台詞ID でも、小文字にして16桁の16進でもないレコードである。2列の公開ファイルの `hello,訳`、6列の `abc,UI,,,UI,訳`、7列の `hello,UI,,,UI,,訳` や `,UI,,,UI,,訳` など。source_en 列の無い形では原文は空と同じに見る。publish はこのレコードを捨てる（R17）ので、書いた訳は公開されない。値がどれも空のレコード（改善の ui-15）と同じ理由である。訳の入ったこのレコードで publish がどうなるかは、入力で違う。作業コピーが入力のときは、集計の malformed dropped に数えるだけで止まらず、訳は黙って落ちる。作業コピーの無いロケールで公開ファイル自身が入力のとき（入力と書き出し先が同じ）は、失われる訳の確かめ（publish.CheckLoss。rowKey が keyDropped の行に訳があれば PublishRowGone）がこのレコードを拾い、publish は1バイトも書かずに止まる（終了コード 1。ほかのロケールも書かない）。画面の書き出し（公開ファイルの形）も同じ確かめで断る（409）。画面はこのレコードの訳を空にする書き換えも断るので、止まった状態は画面からはほどけず、key 列を直すか行を消すのを手で行う。PR3 のはじめは key 列に値のあるこの形（2列の `hello,訳` など）を編集させたので、訳を空にすれば通せた。key 列が空の形（2列の `,訳`）は、そのころから同じように止まっていた。入力による違いは、edit の TestKeylessRowInPublishedFileStopsPublish と web の TestExportRefusesKeylessRowOfPublishedFile が固定する。原文があって key 列と合わないレコード（R15）は、publish が捨てても当てない（diff が「publish で捨てられる行」として知らせる）。判定は publish と同じ関数（publish.Keyless。publish がキーを決める rowKey を呼ぶ）で行い、規則を2か所に書かない。ゲームとの突き合わせの鍵（csvfile.RecordIdentity）は変えない。PR3 のはじめは、この鍵にそろえて key 列も原文も空のレコードだけを当てていたが、key 列に16桁のキーでない値のある原文の空のレコードも publish は捨てるので、publish の規則にそろえた。
- 理由は直す先を指す順に1つだけ付ける。列の数、飲み込み、ゲームの読み方との食い違い、publish がキーを決められないの順である（PR3 では最後に訳の改行があった。PR4 で外した）。キーを決められないことを列の数と飲み込みより後ろに置くのは、どちらもレコードの値の位置が崩れている形で、key 列や原文の列に見えている値が本当の値とは限らないためである（先に直すとキーのあるレコードに戻ることがある）。ゲームの読み方との食い違いは key 列か原文のあるレコードを組にして比べるので、key 列に16桁のキーでない値があるレコードでは同時に当たることがある。食い違いはファイルの形を直す理由なので先に置く（key 列も原文も空のレコードは食い違いの判定に入らず、同時には当たらない）。
- 行の区切りがすべて単独の CR のファイル（csvfile.CROnlyLineBreaks）は、ファイル全体を読み取り専用にする（edit_cr_only）。ゲームはこのファイルを1行と読み、どのレコードも値が割れる。

画面の見せ方（PR3）。画面は1レコードを1行として並べ、行を ID で引く（data-id）。行番号の欄には最初の物理行を出し、行をまたぐレコードだけ、その下に小さく「〜最後の物理行」を添える（決まったことのそのほか 3。区切りの字は目録の ui.line_end）。原文の欄は値の中の改行と空行をそのまま描き（white-space: pre-wrap）、値の中の '#' の行は見出しにしない。409 のあとは、抱えている訳をキーで（キーの無い行は同じ ID へ）載せ直す。載せる先が編集できない行のとき（開いているあいだにファイルが読み取り専用の形に変わったときなど）と、キーの無い行の ID にいまキーのある行が来ているときは載せず、行き先の無い訳として出し続ける（決まったことのそのほか 5）。競合の行の「ファイルの訳」と「あなたの訳」、行き先の無い訳とまだファイルに入っていない訳の一覧は、値ごとに段を分けて出す。

訳への改行の入力（PR4。決まったことの 1）。画面は、どの行でも Shift+Enter で訳に改行（LF）を入れる。Enter はいままでどおり次の行へ進む。変換中は横取りせず、変換を確定した直後の猶予（100ms）を改行にもかける。貼り付けた改行は残し、CRLF と単独の CR を LF にそろえ、NUL を落とし、末尾の改行を1つだけ落とす。edit は訳の CRLF と単独の CR を LF にそろえて書き（決まったことのそのほか 1。作業コピーのレコードの区切りが CRLF でも値の中は LF）、送られた値と違えば画面へ断り（warn.value_normalized）を付ける。改行の入った訳は引用符で囲んで書き（escapeTranslation）、変わるのはそのレコードの最終フィールドだけである。訳が複数行のレコードの読み取り専用（edit_multiline_translation）は外した。改行のぶんだけレコードの物理行の数が変わり、後ろの行の行番号がずれる（ID は変わらない）。edit はモデルの行番号をずらし、書く前の事後確認は、物理行の数の一致の代わりに、入れた書き換えのぶんだけで見積もった行番号と比べる。保存の応答は、行番号が変わったデータ行の ID といまの行番号（numbers）と、数えたもの（stats）を返し、画面は行番号の欄を直す。書こうとした訳の行が、その行だけで読むとレコードに見える（飲み込みの疑い。publish の形の確かめ (f) と同じ関数）ときは、差し替えたレコードをヘッダーと並べて確かめ、訳の何行目かを添えて書かない（edit_line_looks_like_record）。PR3 ではこの形を書く直前の全体の確かめで要求ごと断っていた。書いた複数行の訳は、上流の hash-strings.ps1（pwsh 7.4.6 と 7.6.6）と dwloc publish で出力がバイト一致する（上流との突き合わせの入力の表の ml-translation-* と ml-edit-*）。

書き込みの錠（PR3。改善の決定 3）。画面の保存は、版の照合から rename までを OS の錠（Windows は LockFileEx、Linux・macOS などは flock）で囲む。錠は、利用者のキャッシュのフォルダーの dwloc/locks に、書き出し先の実体の絶対パスから名前を作って置くファイルに掛け、消さずに残す（publish.LockFile）。キャッシュのフォルダーに置けない環境では、一時フォルダーの dwloc-locks-<利用者の番号>（Windows は dwloc-locks）に置く。置けないのは、フォルダーを作れないとき（HOME の無い利用者で動かした docker など）と、フォルダーはあるが錠のファイルを開くと権限で断られるか読み取り専用のファイルシステムのとき（利用者のホームを渡して root で動かした docker が dwloc/locks を root の持ち物で作ったあと、ホームを読み取り専用で渡したコンテナーなど。PR3 の検証で、フォルダーを作れるかだけで決めていたころは保存が毎回 503、publish が毎回終了コード2になった）である。錠のファイルを開けないほかの誤り（開けるファイルの数の上限など、そのときだけのもの）では移らずに止める。移ると、同じ書き出し先を書くほかの dwloc と別の錠のファイルを使い、直列にならない。錠が無かった PR3 の前はその環境でも書けたので、錠のために毎回止めない。一時フォルダーは、自分のもので、ほかの利用者の書けないフォルダーのときだけ使う（/tmp で、ほかの利用者に錠のファイルを消されないため）。どちらにも置けなければ、直し方（向け直す環境変数）を添えて止める。書き出し先の横に置いて放すときに消す形は、翻訳リポジトリに錠のファイルが残りうることと、Windows で消したファイルを開けない・2つが同時に錠を持つことがあったので採らなかった。dwloc publish は、入力・書き出し先・ゲーム側の公開ファイルを1回ずつ読み、形の確かめ・組み立て・土台の食い違い・失われる訳の確かめをどれもそのバイト列で行う。書く直前に、入力と書き出し先へ同じ錠を掛けてから（publish.LockFiles。錠のファイルの名前の順に掛け、同じファイルには1度だけ）3つを読み直し、1バイトでも違えば書かずに止める（終了コード1）。入力だけに錠を掛けると、画面が公開ファイルを開いているとき（--no-game など）の保存と、ゲーム側の作業コピーから同じ公開ファイルを書く publish が直列にならず、画面が保存済みと出した訳を publish が上書きした（PR3 の検証で再現した）。ゲーム側の公開ファイルは dwloc が書かないので、錠は掛けずに読み直すだけにする。上流の道具には無い振る舞いで、上流とのバイト一致は変えない（止めないときの出力は同じ）。錠が効くのは、同じ錠のフォルダーを使う dwloc どうし（同じ利用者の同じ環境）だけである。コンテナーと手元、WSL と Windows、HOME の違う端末、キャッシュのフォルダーに置く dwloc と一時フォルダーへ落とした dwloc は、錠のフォルダーが違うので直列にならない。同じファイルをハードリンクの別の名前から書く道も直列にならない。そうした組み合わせで同じファイルを同時に書くと、守りは版の照合（画面）と書く直前の読み直し（publish）だけになり、読み直してから書き終えるまでの隙に入った書き込みは消えうる（PR3 の検証で、錠の置き場を分けた画面の保存と publish を重ね、200 の saved が返った訳が消えることを再現した）。README（ja・en）の約束も、この条件付きで書く。

diff と画面の比べ方は、値の中の CRLF を LF にそろえてから比べる（決まったことのそのほか 1）。はみ出しの記録を行に結び付けるとき（原文のキーと測ったときの訳）がそれにあたる。publish で捨てられる行のうち、原文の CRLF を LF に戻すと key と一致するものは、注記でそう言う（note_dropped_source_crlf。決まったことのそのほか 7）。報告に出す値は書き換えない。
...
publish の読み方を全体を解釈する読み手へ移す作業（PR0〜PR4）の土台として、上流 main（dc55c9e）の tools/hash-strings.ps1 と dwloc を、合成した入力の表で突き合わせる試験を置いた。入力は testdata/upstream/cases.json（70件、英文と訳はどれも架空の文）、上流の正解は testdata/upstream/expected.json で、scripts/upstream-fixtures.ps1 が上流の Read-Csv と Remove-NonRecords を AST で取り出して読ませた結果（レコードの値と物理行の範囲）と、上流のスクリプトを通しで走らせた結果（出力のバイトと集計の1行）を持つ。作り方は testdata/upstream/README.md にある。

保存した正解は pwsh 7.4.6（上流の docker の hash 経路と同じ。mcr.microsoft.com/dotnet/sdk:8.0-noble に dotnet tool で入れたもの、Linux、カルチャは不変）で作った。手元の pwsh 7.6.6（Windows、ja-JP）で作り直した結果とは、pwsh の版の記録と例外の文面の言語のほかは同じだった（2026-09-24。原文の単独の CR の3件を足して作り直したときも同じだった）。culture に依存する StartsWith('#') も、両方で U+00AD のあとの '#' をコメントと見なした。
...
| ---- | ---- | ---- | ---- |
| 単独の CR（引用の中、行末） | Remove-NonRecords が単独の CR を行末と見なさず、その手前を捨てる。その行の訳が消える | 単独の CR も行の区切りにして読む。引用の中の単独の CR は値に残し、publish は訳の入ったレコードなら形の確かめ (g) で止めて LF に直すよう案内する（決まったことのそのほか 1。原文の列なら、直してよいかをその行のキーの決まり方で分けて案内する） | lone-cr-in-quoted-translation、lone-cr-line-end |
| 原文（source_en）の中の単独の CR | Remove-NonRecords が単独の CR を行末と見なさず、CR の手前を捨て、CR の後ろの残り（閉じ引用符を含む）を最初の列にしたレコードを読む。7列の作業コピー（key 列のキー、台詞ID）ではその行を書かず、2列の作業コピーではその残りから作った別のキー（ゲームが引かないキー）で訳を書く | 引用の中の単独の CR は値に残し、訳の入ったレコードなら形の確かめ (g) で止める。原文を直してよいかを、その行のキーの決まり方で分けて案内する（上の (g)。publish.LoneCRKeyKind。決まったことの 19 と 20）。別のキーでは書かない。2列の作業コピーでは、単独の CR で物理行が分かれ、続きの行がヘッダーと同じ2列に見えるので、飲み込みの確かめ (f) にも当たる。そのレコードを --accept-multiline で通しても (g) で止まる | lone-cr-source-key、lone-cr-source-line-id、lone-cr-source-2col |
...
PR2 のマージの前に、原文（source_en）の中の単独の CR の3件（lone-cr-source-key、lone-cr-source-line-id、lone-cr-source-2col）を入力の表に足した（決まったことの 19）。どれも上の表の「原文（source_en）の中の単独の CR」の行で、意図して違える点に置く。読み手の試験では、ほかの単独の CR と同じく、上流が CR の手前を捨てる読み方の違いとして表に載る。
