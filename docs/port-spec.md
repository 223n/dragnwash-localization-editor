...
書き込みの錠（PR3。改善の決定 3）。画面の保存は、版の照合から rename までを OS の錠（Windows は LockFileEx、Linux・macOS などは flock）で囲む。錠は、利用者のキャッシュのフォルダーの dwloc/locks に、書き出し先の実体の絶対パスから名前を作って置くファイルに掛け、消さずに残す（publish.LockFile）。キャッシュのフォルダーを作れない環境（HOME の無い利用者で動かした docker など）では、一時フォルダーの dwloc-locks-<利用者の番号>（Windows は dwloc-locks）に置く。錠が無かった PR3 の前はその環境でも書けたので、錠のために毎回止めない。一時フォルダーは、自分のもので、ほかの利用者の書けないフォルダーのときだけ使う（/tmp で、ほかの利用者に錠のファイルを消されないため）。どちらも作れなければ、直し方（向け直す環境変数）を添えて止める。書き出し先の横に置いて放すときに消す形は、翻訳リポジトリに錠のファイルが残りうることと、Windows で消したファイルを開けない・2つが同時に錠を持つことがあったので採らなかった。dwloc publish は、入力・書き出し先・ゲーム側の公開ファイルを1回ずつ読み、形の確かめ・組み立て・土台の食い違い・失われる訳の確かめをどれもそのバイト列で行う。書く直前に、入力と書き出し先へ同じ錠を掛けてから（publish.LockFiles。錠のファイルの名前の順に掛け、同じファイルには1度だけ）3つを読み直し、1バイトでも違えば書かずに止める（終了コード1）。入力だけに錠を掛けると、画面が公開ファイルを開いているとき（--no-game など）の保存と、ゲーム側の作業コピーから同じ公開ファイルを書く publish が直列にならず、画面が保存済みと出した訳を publish が上書きした（PR3 の検証で再現した）。ゲーム側の公開ファイルは dwloc が書かないので、錠は掛けずに読み直すだけにする。上流の道具には無い振る舞いで、上流とのバイト一致は変えない（止めないときの出力は同じ）。
