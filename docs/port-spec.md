...
- 最初の列名が '#' で始まるヘッダー（`"#key"` など）は、上流と違って飛ばさずにヘッダーとして返す。呼び出し側は PowerShellHeader.CommentLike で見分け、publish は PR2 で (a) に足して止める。
