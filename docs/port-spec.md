...
全体を解釈する読み手へ移す作業（PR0〜PR4）の PR1 で、internal/csvfile に読み方の土台を足した。PR2 で publish（集め方・失われる訳の確かめ・土台の確かめ・形の確かめ）、diff（ReadRows と ParseLayoutRisks）、order（LoadPowerShell）がこの読み手へ移った（行だけが要る使い手は PowerShellFile.Rows を使う）。edit は PR2 で行の種類（見出し・空行・ヘッダー・データ行）を区切りの関数で決めるようにした。保存の単位は物理行のままで、レコードの単位へ移すのは PR3 である（下の「使い手の扱い」）。行単位の読み手（csvfile.ReadPowerShellRows・ReadPowerShellRowsNumbered・ReadPowerShellTable）と、PR1 まで publish の守りが使っていた csvfile.ReadPowerShellWhole は、どの使い手も使わなくなり、回帰試験と上流との突き合わせの表のためにだけ残す。ここに書くのは、その土台の約束である。
...
- edit（画面の保存）: ファイル全体を読み取り専用にし（reason edit_unclosed_quote）、引用符が開いたレコードから後ろを、レコードとして解釈しない物理行のまま並べる。行をまたぐレコード（閉じている）は、どの物理行も理由（edit_multiline）を付けて編集させない。保存が物理行の単位なので、1行目へ書くと続きの行が残り、publish の読み方では壊れたレコードになるためである。1行目には全体を解釈して読んだ値を持たせ、キーと原文の全体を画面に出す。ヘッダーの受理は生テキストの完全一致のまま残し、どのセグメントをヘッダーにするかだけを区切りの関数にそろえた（決まったことのそのほか 8。ヘッダーの前の "," の行はヘッダーになって受理されず、全角空白だけの行は空行になる）。
