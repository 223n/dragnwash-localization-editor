...
- publish がキーを決められない（edit_no_key_or_source。決まったことの 24）。原文（source_en。トリムしない）が空で、key 列（前後の空白を除く）が台詞ID でも、小文字にして16桁の16進でもないレコードである。2列の公開ファイルの `hello,訳`、6列の `abc,UI,,,UI,訳`、7列の `hello,UI,,,UI,,訳` や `,UI,,,UI,,訳` など。source_en 列の無い形では原文は空と同じに見る。publish はこのレコードを捨てる（R17。集計の malformed dropped に数える）ので、書いた訳は公開されず、黙って落ちる。値がどれも空のレコード（改善の ui-15）と同じ理由である。原文があって key 列と合わないレコード（R15）は、publish が捨てても当てない（diff が「publish で捨てられる行」として知らせる）。判定は publish と同じ関数（publish.Keyless。publish がキーを決める rowKey を呼ぶ）で行い、規則を2か所に書かない。ゲームとの突き合わせの鍵（csvfile.RecordIdentity）は変えない。PR3 のはじめは、この鍵にそろえて key 列も原文も空のレコードだけを当てていたが、key 列に16桁のキーでない値のある原文の空のレコードも publish は捨てるので、publish の規則にそろえた。
