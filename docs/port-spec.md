...
- 理由は直す先を指す順に1つだけ付ける。列の数、飲み込み、ゲームの読み方との食い違い、publish がキーを決められない、訳の改行の順である。キーを決められないことを列の数と飲み込みより後ろに置くのは、どちらもレコードの値の位置が崩れている形で、key 列や原文の列に見えている値が本当の値とは限らないためである（先に直すとキーのあるレコードに戻ることがある）。ゲームの読み方との食い違いは key 列か原文のあるレコードを組にして比べるので、key 列に16桁のキーでない値があるレコードでは同時に当たることがある。食い違いはファイルの形を直す理由なので先に置く（key 列も原文も空のレコードは食い違いの判定に入らず、同時には当たらない）。訳の改行より前に置くのは、訳の改行が PR4 までの画面の制限で、キーを決められないことは PR4 のあとも publish が捨てることに変わりがないためである。
- 行の区切りがすべて単独の CR のファイル（csvfile.CROnlyLineBreaks）は、ファイル全体を読み取り専用にする（edit_cr_only）。ゲームはこのファイルを1行と読み、どのレコードも値が割れる。

画面の見せ方（PR3）。画面は1レコードを1行として並べ、行を ID で引く（data-id）。行番号の欄には最初の物理行を出し、行をまたぐレコードだけ、その下に小さく「〜最後の物理行」を添える（決まったことのそのほか 3。区切りの字は目録の ui.line_end）。原文の欄は値の中の改行と空行をそのまま描き（white-space: pre-wrap）、値の中の '#' の行は見出しにしない。409 のあとは、抱えている訳をキーで（キーの無い行は同じ ID へ）載せ直す。載せる先が編集できない行のとき（開いているあいだにファイルが読み取り専用の形に変わったときなど）と、キーの無い行の ID にいまキーのある行が来ているときは載せず、行き先の無い訳として出し続ける（決まったことのそのほか 5）。競合の行の「ファイルの訳」と「あなたの訳」、行き先の無い訳とまだファイルに入っていない訳の一覧は、値ごとに段を分けて出す。訳への改行の入力はまだ無い（PR4）。
...
publish の読み方を全体を解釈する読み手へ移す作業（PR0〜PR4）の土台として、上流 main（dc55c9e）の tools/hash-strings.ps1 と dwloc を、合成した入力の表で突き合わせる試験を置いた。入力は testdata/upstream/cases.json（68件、英文と訳はどれも架空の文）、上流の正解は testdata/upstream/expected.json で、scripts/upstream-fixtures.ps1 が上流の Read-Csv と Remove-NonRecords を AST で取り出して読ませた結果（レコードの値と物理行の範囲）と、上流のスクリプトを通しで走らせた結果（出力のバイトと集計の1行）を持つ。作り方は testdata/upstream/README.md にある。
