...
「行をまたぐ」と「閉じない」は、行ごとの引用符の偶奇ではなく、ConvertFrom-Csv の全文の読み方で決める（csvfile.ReadPowerShellWhole。規則は区切りの関数 csvfile.SplitSegments が持つ。下の「全体を解釈する読み手（csvfile）」）。pwsh 7.6.6 で実測した規則は、引用の中の LF・CRLF・単独の CR は値に残る、フィールドの途中の '"' と閉じ引用符の後ろの '"' は引用を開かない、先頭の空白の後ろの '"' は引用を開く、閉じない引用符はファイルの終わりまでを値にする、の4つ。引用符なしのフィールドの途中の '"'（5" screen のような値）は、行単位で読んでも全体を読んでも値が同じなので止めない（上流の報告 #4b の入力 p-bare-quote）。行の区切りは上流の Remove-NonRecords の正規表現ではなく、行単位の読み手と同じ \r\n / \n / \r にした。こうしておくと「行をまたぐ」が「行単位の読み手が値を切る」とちょうど重なる。
...
残る隙間: 引用符で囲まない値の中の単独の CR（手で書いた `a\rb` など）は、行単位の読み手も全文の読み方も行の区切りとして値を切るが、引用が無いのでいまの確かめのどれにも当たらない。ゲームは引用の外の CR を捨てて ab と読む（R8）。書き手（Escape-Csv）は CR を含む値を必ず引用するので、道具が書いたファイルには現れない。いまは切れた訳をそのまま公開する。PR2 で止める（下の「上流と意図して違える点」の表。見つける関数は csvfile.FindCRCuts）。

### 全体を解釈する読み手（csvfile）

全体を解釈する読み手へ移す作業（PR0〜PR4）の PR1 で、internal/csvfile に読み方の土台を足した。publish・diff・order・edit・画面の読み手はまだ替えていない（PR2 以降）。ここに書くのは、その土台の約束である。
...
- 最初の列名が '#' で始まるヘッダー（`"#key"` など）は、上流と違って飛ばさずにヘッダーとして返す。呼び出し側は PowerShellHeader.CommentLike で見分け、publish は PR2 で (a) に足して止める。CommentLike は列名の前後の空白を除いて見る。上流の ConvertFrom-Csv がヘッダーを飛ばすのは、読んだ最初の値そのものが '#' で始まるとき（`"#key"`、先頭の半角空白を読み手が落とす ` #key`）だけなので、引用の中の空白（`" #key"`）や、読み手が落とさない NO-BREAK SPACE の後ろに '#' があるヘッダーは、上流ではその名前の列になる。dwloc はこれらも止める（下の表）。
...
- 引用符で囲まない値の中の単独の CR（csvfile.FindCRCuts）: 単独の CR で終わるレコードのうち、次のセグメント（区切りの関数が分けたもの）がレコードに見えないものを返す。次のセグメントが空行（空白だけの行を含む）・'#' の行・空のレコード（"," や '""'）なら返さない。CR だけの改行のファイルは、見出しのコメント行や空行、空のレコードの前でも単独の CR で改行するので、そこで止めると意図して読んでいる形が通らなくなるためである。その代わり、値が CR の直後の '#' で切れる形は見逃す。次のセグメントがレコードなら、最初の値がキーの形か、区切りの数がヘッダーの列数と同じならレコードに見えるとする（飲み込みと同じ見方）。区切りはレコード全体で数える。次の物理行だけを単独で読むと、キー列の空いた行の原文が行をまたぐとき、引用が開いたまま行が終わって区切りが足りず、正当な CR だけのファイルで当たるためである。入力の表の改行をすべて単独の CR に直した写しでは、切れた値（lone-cr-unquoted-value）のほかに、U+00AD のあとの '#' の行（soft-hyphen-comment。読み手はデータのレコードにする）と、ヘッダーより列の足りないレコード（ml-header）の前の CR でも当たる。どちらも切れた値ではないが、単独の CR の後ろでは見分けられない。PR2 で止める前に扱いを決める（未決）。
...
| 引用符で囲まない値の中の単独の CR（上の「残る隙間」） | その行を失う | 単独の CR を行の区切りにする規則のまま読み、値は CR の手前で切れる。いまは切れた訳を止めずに公開し、続きの物理行は malformed dropped に数えるだけで知らせない（ゲームは引用の外の CR を捨てて、切れる前の値を読む）。PR2 で、引用の外で単独の CR で終わるレコードの次のセグメントが、レコードに見えなければ止め、CR を取り除くか値を引用符で囲むよう案内する（csvfile.FindCRCuts）。次のセグメントが空行・'#' の行・空のレコードなら止めない（CR だけの改行のファイルを読むため）。行末の単独の CR で次の行がレコードになる形（lone-cr-line-end）は、いまどおり読む | lone-cr-unquoted-value |
...
| 引用の中の空白か NO-BREAK SPACE の後ろに '#' があるヘッダー | 読んだ最初の値が '#' で始まらないので飛ばさず、その名前（` #key` など）の列にする。key 列が無いので source_en から訳を書く | 読み方は上流と同じで、いまの publish も上流と同じバイトを書く。判定（PowerShellHeader.CommentLike）は、決めた判定の文面（最初の列名の前後の空白を除いて '#' で始まる）どおり空白を除いて見るので、'#' で始まると見なし、PR2 から (a) で止める。上流の飛ばし方（読んだ最初の値そのもの）にはそろえていない | hash-header-quoted-space、hash-header-nbsp |
...
PR0 では「引用符で囲まない値の中の単独の CR」もこの表に置いていた。PR1 の時点で PR2 で止めると決まったので、「上流と意図して違える点」の表へ移した。publish の試験は、いまの「切れた訳を公開する」を「PR2 で変わる」として固定する。
