...
書き込みの錠（PR3。改善の決定 3）。画面の保存は、版の照合から rename までを OS の錠（Windows は LockFileEx、Linux・macOS などは flock）で囲む。錠は書き出し先の横に置く <名前>.dwloc-lock に掛け、放すときに消す（publish.LockFile）。dwloc publish は、組み立てに使った入力のバイト列を覚え、書く直前に入力へ同じ錠を掛けてから読み直し、違えば1バイトも書かずに止める（終了コード1）。上流の道具には無い振る舞いで、上流とのバイト一致は変えない（止めないときの出力は同じ）。
