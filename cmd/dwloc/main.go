...
// ファイルを読めずに判定しなかったカテゴリがあるとき、publish が「書くと訳が
// 失われる」「読み違える形のファイルがある」「ゲームに入っている翻訳が古い」
// 「組み立てたあとに入力か書き出し先が変わった」と判断して1バイトも書かずに
// 止まったとき、そして publish --check が書き換えの要るロケールを見つけたときです
// （README の終了コードの8つ）。
...
const (
	...
	// publish --check が書き換えの要るロケールを見つけたとき。
	// 実行そのものは成功している。
	exitProblems = 1
...
終了コード:
  ...
  1   validate が問題を見つけた、diff が要確認を見つけた（diff --strict では
      ...
      いないカテゴリがある、publish が「書くと訳が失われる」「読み違える
      形のファイルがある」「ゲームに入っている翻訳が古い」「組み立てたあとに
      入力か書き出し先が変わった」と判断して止まった、または
      publish --check で書き換えが要るロケールがある
