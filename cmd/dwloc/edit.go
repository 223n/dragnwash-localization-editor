...
publish は回しません。保存は「触ったレコードの最終フィールドだけを差し替える」処理で、
再生成ではありません。触っていない行は1バイトも変わりません。引用符で囲んだ値が
行をまたぐレコードも1件として扱います。

保存の前にファイルの版を照合します。手前でファイルが変わっていたら書かず、
画面に読み直させて、どちらの編集を残すかを選ばせます。照合から書き終えるまでは
OS の錠で囲むので、同じ環境で dwloc edit を2つ動かしても、publish を同時に回しても、
保存した訳は消えません（publish は入力と書き出し先の両方に同じ錠を掛けます）。
錠は利用者のキャッシュのフォルダーに置くので、コンテナーと手元、WSL と Windows の
ように環境をまたいで同じファイルを同時に書くと効きません。片方を終えてから
もう片方を動かしてください。
