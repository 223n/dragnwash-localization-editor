...
承認待ちの実行は取り消せません（HTTP 409）。
書き込み権限のある人のトークンなら、ボタンの代わりにAPIでも承認できます。
「リリース」を`auto_merge`で動かしたときは、ワークフローが承認待ちの実行をAPIで承認し、CIが通ってからマージします。
承認を拒まれたときは、`release/*`ブランチでCIを動かして代えます。
その場合、承認待ちの実行は期限切れの失敗として残ります。
