...
| `key`列も原文（`source_en`）も空の行（`,UI,,,UI,,訳`など）                 | key列も原文（source_en）も空（publishがこのレコードを捨てるので、書いた訳は公開されない） |
...
キーと原文の両方が空で、ほかの列（訳など）に値がある行は並べますが、同じ理由で書き換えられません（表の最後の行）。  
公開ファイルの形（`source_en`列の無い形）では、`key`列が空の行がこれに当たります。  
その訳を残したいときは、キーのある行へ訳を移してから、要らない行を消してください。
