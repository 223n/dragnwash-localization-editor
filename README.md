...
ブラウザーで落とした`.tar.gz`には、隔離の印（`com.apple.quarantine`）が付きます。  
ターミナルの`tar`で展開しても、この印は中の`./dwloc`へ移ります。  
印が付いたままだと、Gatekeeperが実行を止めます。  
そのため、`xattr`の行は省かないでください。

`xattr`の行を飛ばして`./dwloc version`を実行すると、「“dwloc”は開いていません」という窓が出ます。  
そのときは「完了」を押し、`xattr`の行を実行してから`./dwloc version`をやり直します。  
「ゴミ箱に入れる」を押すと、展開した`dwloc`がゴミ箱へ移ります。  
その場合は、`.tar.gz`から展開し直してください。

`gh release download`で落としたときは、印が付きませんでした。  
そのときは`xattr`の行が`No such xattr`と出ます。  
それでかまいません。  
そのまま次へ進みます。
...
> [!NOTE]
> macOS 27.2（Apple Silicon）で、`v0.11.0`の`darwin_arm64`の書庫を使って確かめました（2026年9月26日）。  
> ブラウザーで落としたときの印は、`gh release download`で落とした書庫へ手で付けて再現しています。  
> Intel版（`darwin_amd64`）と、Finderで展開したときは確かめていません。
...
Macのキーボードでは、`fn`を押しながら`F1`を押します。  
`F1`だけでは、窓が開きませんでした（キーボードの設定が既定のままのMacで確かめました）。  
システム設定の「キーボード」で「F1、F2などのキーを標準のファンクションキーとして使用」をオンにしていれば、`fn`は要らないはずです。  
こちらは確かめていません。
...
キーの形で始まるか、カンマで区切った列の数がヘッダーの列の数と同じなら（7列の作業コピーならカンマ6個）、閉じ忘れを疑います。  
...
1. ゲームが動いていれば、約2秒でホットリロードが読み直します。
   動いていなければ、1度起動します。
   macOSでは、ゲームの窓が裏にあるあいだは読み直さないことがあります（下の「画面で訳を書き換える」）
...
実験的な方法でModを入れたMacでは、次のことを確かめました。

- `--game auto`がゲームのフォルダーを見つけ、`diff`が作業コピーを読みます
- `edit`で原文の欄が埋まり、保存すると作業コピーのその行だけが変わります
- `publish --game auto`が終了コード`0`で通り、`edit`で直した訳が公開ファイルに入ります

> [!NOTE]
> 確かめた環境は、macOS 27.2（Apple Silicon）、Doorstop 4.6.0、ModFramework 1.5.0です（2026年9月26日）。  
> `dwloc`は`v0.11.0`のあとの開発版です。  
> Intel Mac（`darwin_amd64`）では確かめていません。
...
macOSで確かめたときは、ゲームの窓が裏にあるあいだは読み直しませんでした。  
ゲームの窓を前に出すと、そこで読み直しました。  
ブラウザーで訳を打ったあと、反映されないときは、ゲームの窓を一度前に出してください。  
Windowsで同じことが起きるかは確かめていません。
...
訳に改行を入れるときは、`Shift+Enter`を押します。  
どの行でも入れられます。  
`Enter`はいままでどおり、次の行へ進みます。  
変換を確定した直後の`Shift+Enter`では改行を入れません（確定しただけで改行が入らないようにするためです）。  
貼り付けた文の改行も、そのまま残します。  
`CRLF`や`CR`だけの改行は`LF`にそろえます。  
貼り付けた文の末尾の改行は、1つだけ落とします。  
表計算ソフトのセルやエディターの行を写すと、行の終わりの改行が付いてくるためです。  
末尾に改行が要るときは、貼り付けたあとに`Shift+Enter`で足してください。

改行の入った訳は、引用符で囲んだ値として書きます。  
値の中の改行は`LF`です。  
作業コピーの行の区切りが`CRLF`でも、値の中は`LF`にします（ゲームが書く作業コピーと同じ形です）。  
訳の2行目以降に、その行だけで読むとレコードに見える行（16桁のキーや台詞IDで始まる行や、カンマで区切った列の数がヘッダーの列の数と同じになる行。7列の作業コピーならカンマ6個の行）があると、その訳は書きません。  
`publish`が、引用符の閉じ誤りで後ろの行を飲み込んでいる疑いとして止める形だからです。  
行には「訳のN行目を1行だけで読むとレコードに見える」という理由が出て、打った訳は画面に残ります。  
その行の書き方（カンマなど）を変えてから、もう一度打ってください。
...
原文が行をまたぐレコードも、訳に改行があるレコードも書き換えられます。  
...
訳に改行を足したり消したりして保存すると、そのレコードと後ろのレコードの行番号がずれます。  
画面は保存のたびに、ずれた行の行番号と、「数えたもの」のファイルの物理行を直します。  
原文の欄と訳の欄は、値の中の改行と空行をそのまま出します。

保存する行は、ファイルの行番号ではなく、レコードの通し番号とキーで指します。  
行をまたぐレコードがあると、後ろのレコードの行番号がずれるためです。  
自分の保存では、訳に改行を足しても通し番号は変わりません。  
...
| `Shift+Enter` | 訳に改行を入れます。行は移りません。変換を確定した直後は入れません。`Ctrl`や`Alt`も一緒に押したときは改行を入れず、`Enter`と同じく次の行へ進みます |
| `Escape`   | 入力欄を閉じ、その行の訳の欄に留まります。打った訳は残ります。そこで`Enter`を押すと開き直します                                                  |
| `Tab`      | 次の行へ移ります                                                                                                                                 |
...
`Escape`で入力欄を閉じて、焦点がその行の訳の欄に留まっているあいだも隠しません。  
隠すと、直すべき行と触っている行が画面から消えるためです。

入力欄を閉じると、その行が条件に当たらないときはそこで隠れます。  
上から順に`Enter`で打っていくときは隠れません。  
閉じる時点ではまだ未保存なので、「未保存の訳がある行」として残るためです。  
`Escape`で閉じたときは、焦点がその行の訳の欄から離れたところで隠れます。  
留まっている欄を隠すと焦点の行き場が無くなり、`Enter`で開き直せなくなるためです。
...
2026年9月27日の時点では、`main`と`dev`の版は同じです。  
どちらの版も、`credits.txt`の状態語`native`を受け付けます。  
上流の不具合を写していない入力（コメント行の引用符など）では、`dwloc validate`だけが問題を報告することがあります。
...
macOSの実機でも、実験的な方法でModを入れたゲームを相手に、`diff`・`edit`・`publish`を確かめています（`v0.11.0`のあとの開発版、2026年9月26日）。  
...
1. `auto_merge`を有効にすると、Pull Requestの確認を挟まずにマージし、公開まで一気に進みます。  
   人が`Approve workflows to run`を押す代わりに、ワークフローがPull Requestの承認待ちの実行をAPIで承認します。  
   承認には`GITHUB_TOKEN`を使います。  
   v0.12.1で、承認が通って実行が動き出すことを確かめました。  
   CIの実行が動き出したら、通るのを最大30分待ってからマージします。  
   承認を拒まれたときは、`release/vX.Y.Z`ブランチでCI（`ci.yml`）を動かし、通るのを最大30分待ってからマージします。  
   この場合、Pull Requestの実行は承認待ちのまま、マージのあとに期限切れの失敗として残ります。  
   どちらになったかは、その実行の要約に出ます。  
   `main`に必須のチェックや承認のルールがあると、マージの段で止まることがあります。  
   CIが落ちたときや、30分で終わらないときも、Pull Requestを開いたまま止まります。  
   止まったときは、人がCIの通ったことを確かめてからマージすれば、公開まで進みます。  
   Pull Requestに`Approve workflows to run`が出ていれば、押してCIが通ってからにしてください。  
   押さずにマージすると、承認待ちの実行が期限切れの失敗として残ります。  
   CIの揺れで止まったときは、「リリース」の実行で`Re-run failed jobs`を押しても進められます。  
   承認待ちの実行が残っていれば、承認からやり直します。  
   Pull RequestのCIをすでに承認していたときは、`release/vX.Y.Z`ブランチでCIを動かし直します。  
   CIが通れば、マージと公開まで進みます。  
...
ボタンの代わりに、APIでも承認できます（v0.12.0で確かめました）。  
手順は「ワークフローが開いたPull RequestのCI」にあります。
...
| 経路           | すること                                                                                                                                    |
|----------------|---------------------------------------------------------------------------------------------------------------------------------------------|
| 人がマージする | `Approve workflows to run`を押し、CIが通ってからマージします。ボタンの代わりにAPIでも承認できます                                           |
| `auto_merge`   | ワークフローが承認待ちの実行をAPIで承認し、CIが通ってからマージします。承認を拒まれたときは、`release/vX.Y.Z`ブランチでCIを動かして代えます |
...
承認待ちの実行は、APIでは`status`が`completed`、`conclusion`が`action_required`として返ります。  
v0.12.0の2つのPull Request（リリースと`develop`への戻し）で確かめました。  
終わった実行の扱いなので、取り消し（`cancel`）は`Cannot cancel a workflow run that is completed.`（HTTP 409）で拒まれます。  
v0.12.0の`auto_merge`は、こうして3件を取り消せず、期限切れの失敗として残しました。

取り消しの代わりに、承認（`approve`）を使います。  
利用者の`gh`のトークンで承認すると、同じ実行が2回目の試行として動き出しました（`run_attempt`が2になります）。  
v0.12.0で`develop`へ戻したPull Requestの3件で確かめました。  
その試行を起こした人（`triggering_actor`）は、承認した人になります。

`GITHUB_TOKEN`でも承認できます。  
v0.12.1の`auto_merge`で、リリースのPull Requestの3件（CI、CodeQL、「PRのheadブランチを確かめる」）を承認できました。  
3件とも2回目の試行として動き出し、その試行を起こしたのは`github-actions[bot]`でした。  
3件とも通り、期限切れの失敗は残りませんでした。  
このとき`release/vX.Y.Z`ブランチのCIは動かさず、リリースする木のCIは1回だけ走りました。

ただし、これはGitHubの文書に書かれた振る舞いではありません。  
文書は、承認のAPIを公開のフォークから初めて貢献する人のPull Request向けと説明しています。  
権限の表には、GitHub Appのインストールトークン（`GITHUB_TOKEN`もこの一種です）で使え、Actionsの書き込み権限が要ると載っています。  
ワークフロー自身が開いたPull Requestの実行を承認できるかは、書かれていません。  
GitHubが振る舞いを変えると、拒まれるようになる恐れがあります。  
そのため`auto_merge`の経路は、承認を拒まれても止まらずに、`release/vX.Y.Z`ブランチのCIで代えます。  
承認は受け付けられても、実行が動き出さない場合も同じです。  
結果は、その実行の要約に、承認して動き出した件数と拒まれた件数として出ます。

手で承認するときは、次のコマンドを使います。  
`develop`へ戻すPull Requestも、同じ手順で承認できます。
...
# 状態を見る。動き出すとrun_attemptが2になる
gh api "repos/{owner}/{repo}/actions/runs/<実行のid>" --jq '[.status, .conclusion, .run_attempt] | @tsv'
```

`{owner}`と`{repo}`は、`gh`がカレントディレクトリのリポジトリに置き換えます。  
承認に使うのは、書き込み権限のある人のトークンです。
...
| `release.yml`         | 手動                                                                                                                                                                                                            | `develop`の先端でCIが通ったことを確かめてから、リリースブランチを切り、版を上げ、`main`へのPull Requestを開きます。`auto_merge`を指定したときは、Pull Requestの承認待ちの実行をAPIで承認してCIが通るのを待ち、マージして公開まで進めます。承認できなければ、リリースブランチでCIを動かして待ちます |
