...
付けると、何も書かずに終わります（終了コード2）。
理由は下の「publishはゲームのフォルダーを読みません」に書きました。
...
実際に読んだファイルは、`diff`では「作業コピー」の行に、`edit`では画面の「ファイル」の欄に出ます。
...
| `edit`の保存先は1つです | `edit`の保存先は2つになります |
...
`--game`を付けていて、そのロケールの作業コピーがゲーム側にあるときです。
保存のたびに、2つのファイルの「その行」を差し替えます。
どちらも差し替えであって、作り直しではありません。

| 順 | ファイル | 役目 |
| ---- | ---- | ---- |
| 1 | `Translations/<locale>/strings.csv` | コミットする側 |
| 2 | `<ゲーム>/Translations/_discovered/<locale>.working.csv` | ホットリロードする側 |

この順に書きます。
1が失われてはいけないもので、2は失っても「ゲームがホットリロードしない」で済むためです。

- 1が書けなければ、2は書きません。画面は未保存のまま抱えて、送り直します
- 1が書けて2が書けなかったときは、訳はコミットする側に入っています。行ごとに「ゲームには届いていません」と出ます
- 公開ファイルにまだ無い行は、2にだけ入ります。行ごとに断りが出ます

行の対応は、行番号ではなくキー（先頭フィールド）で取ります。
作業コピーと公開ファイルは別々に作られたファイルなので、見出しの数と並びが違うためです。

ゲーム側の作業コピーへ書くと、ゲームが約2秒でホットリロードします。

画面には、並べているファイルと、コミットする側のファイルの両方を出します。
どちらを直しているのかを、画面から確かめられます。

#### publishはゲームのフォルダーを読みません

`publish`は、入力から公開ファイルを作り直す処理です。
入力にゲーム側の作業コピーを選ぶと、そこに無い行と訳が空の行が、コミット済みの公開ファイルから消えます。

実測です。
実機の`ja.working.csv`（1979行）を入力に据えて`publish`を通しました。
元の`Translations/ja/strings.csv`は191,642バイト・1721行です。

| 入力にした作業コピー | `ja/strings.csv` | 終了コード |
| ---- | ---- | ---- |
| 途中まで（1979行のうち400行） | 191,642 → 38,753バイト | 0 |
| ヘッダーの引用符が閉じていない | 191,642 → 43バイト | 0 |

後者は`source_en`と`translation`が1つの列名に融合し、`translation`列を引けなくなります。
全1712行が「訳が空」と見なされて落ち、残ったのはヘッダー1行だけでした。
どちらも終了コードは0なので、失敗として気づけません。

正常な作業コピーでも同じではありません。
行は1721行のまま消えませんが、`speaker`列が17行変わり（`Ryan`と`Conrad`が`UI`になります）、`UI`の行が10行入れ替わります。
作業コピーは、再生順に無い行の`speaker`を持たないためです。

ゲームで直した訳をコミットする側へ入れるのは`edit --game`の仕事です。
上の「editは2つのファイルへ書きます」を見てください。
...
ゲームが`C:/Program Files (x86)/`の下にあると、保存を拒まれることがあります。
...
```text
ゲーム更新 → ゲーム内で Export game flow → dwloc diff --game auto → dwloc edit --game auto で訳を直す → dwloc publish → dwloc validate
```
