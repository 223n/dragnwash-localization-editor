...
キーの欄と原文がどちらも空の行の訳を書き換えると、ゲームがほかの行を見つけられなくなるファイルがあるためです。  
...
どちらも作れないときは、保存と`publish`は書かずに止まり、どの環境変数を直せばよいかを出します。  
...
見出しはいつも描くので、読み上げソフトの見出しの一覧には、画面の外の節と節点も名前つきで並びます。  
ただしChromiumでは、画面の外の行の原文と訳は、見えるところへ来て描くまで読み上げソフトへ渡りません。  
行を読み進めるときは、見出しへ移る操作や`Tab`で、読みたい行を画面へ送ってください。  
