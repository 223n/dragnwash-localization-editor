...
| 操作       | 何が起きるか                                                                                                                                     |
|------------|--------------------------------------------------------------------------------------------------------------------------------------------------|
| 絞り込み   | 選んだ条件のどれかに当たった行だけを出します。複数選べます。条件は、起動時の件数から作った11個と、「画面の状態」の2個（未保存・保存できない）です |
...
絞り込みの11個は、`dwloc diff`が数えるカテゴリと同じものです。  
`diff`は同じ11個を「要作業」「要確認」「参考」の3段に分け、それぞれ理由を添えて出します。  
...
作業コピーを読めないときは、未翻訳・「`publish`で捨てられる行」・「原文とタグが違う行」が「判定していません」になります。  
