...
このうち`edit`だけは、`--game`を省いてもゲームのフォルダーを探します。
...
`publish`と`diff`は探しません。  
`publish`は「何をコミットするか」を決める処理で、`diff`の出力（とくに`--format csv`）は別のPCや機械と突き合わせる使い方があります。  
入力が実行するPCで変わると、どちらも根拠として使えなくなります。  
`edit`は、その場の1人がいま直すための画面なので、そこだけ扱いを変えました。
...
3つとも同じ探し方です。  
一度は`edit`だけが探す形にしていました。`publish`の入力が実行するPCで変わるのを避けたかったためです。
...
同じ引数で`diff`が「作業コピーがありません」、`edit`が「未翻訳32件」と逆のことを言う形にもなっていました。
...
探して見つからなかったときの案内は`edit`だけが出します。  
...
作業コピーは2か所にありえます。  
探す順は次のとおりで、`publish`と`edit`で同じです。
...
黙ったままにはなりません。  
実際に読んだファイルは、`edit`では画面の「ファイル」の欄に、`diff`では「作業コピー」の行に、`publish`ではロケールごとの1行に出ます。
...
片側にしか無い行も数えません。版が違えば行の増減は当たり前に起きるためで、それで訳が消えるなら1つ上の確認が捕まえます。

ゲーム側に公開ファイルが無いときは、そろっているかどうかを判断できないので、この確認は飛ばします。

**残る隙間があります。**  
ゲームへ最新の翻訳を入れ直したあと、ゲームを起動し直さずに`publish --game`を回すと、この確認は通ります。  
土台はそろっているのに、作業コピーだけが入れ直す前に書き出されたままだからです。  
翻訳を入れ直したら、ゲームを1度起動して`Export game flow`をやり直してください。
...
`edit`は`--game`を省いても探すので、そのときも探して無かったことを1行出してから、原文の欄が空のまま始めます。  
...
起動したときの状態の一帯（ゲームのフォルダー、断り書き、件数、数えたもの）だけは、畳みきっていません。  
「ファイル」の欄だけは見出しに残してあり、畳んだままでも読めます。  
...
絞り込みの条件には、いま並べている一覧にその条件の行が何行あるかを添えてあります。  
...
一致しないカテゴリには、件数の欄に「32件」と「この一覧には0行」の両方を出します。  
...
`diff`と`publish`には`--game auto`を付けてください。  
原文の入った作業コピーは、ゲームのフォルダーにしかないためです。  
`edit`は付けなくても探しますが、この並びでは3つそろえたほうが、どこを読んでいるか見て分かります。  
