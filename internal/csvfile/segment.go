...
//
// 読み方の規則は、この関数だけが持つ。主の読み手（[ReadPowerShell]）、守り専用の
// [ReadPowerShellWhole]、飲み込みの検出（[FindSwallows]）は、どれもこの結果の上に
// 載っている。保存（internal/edit）も、ここで分けたレコードの最終フィールドを
// 差し替える形へ移す。書く側と読む側で区切りが割れると、訳ではない値を
