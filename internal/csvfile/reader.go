...
// ` #key` のように、引用符や空白の後ろに '#' があるヘッダーだけである。上流の
// ConvertFrom-Csv はそのレコードを飛ばし、次のレコード（データ）をヘッダーにする。
// そのため、そのファイルの訳は上流では1行も公開されない。dwloc はこの読み方を写さず、
// ヘッダーとして返したうえで、呼び出し側（publish の形の確かめ (a)）がこれを見て
