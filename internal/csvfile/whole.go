...
// 引用の外にある空行・空白だけの行・'#' で始まる行を落とす。この関数も同じ行を
// 落として読むが、「引用の外か」の決め方などは下に挙げるとおり上流と違える。
//
...
//     先頭で開いた引用だけで決める（ConvertFrom-Csv と同じ規則）。上流の
//     Remove-NonRecords は偶奇で決めるので、引用符なしのフィールドの途中に
//     裸の '"' がある行（5" screen のような値）の後ろでは、引用の中にいると
//     見なす。そのため、その後ろのコメント行と空行を上流は落とさずに残し、
//     ConvertFrom-Csv がレコードとして読む（列の多いコメント行は、公開ファイルに
//     不正な行として出ることがある）。ここでは引用の外として落とす。上流の
//     不具合として扱い、写さない。
//
