...
(function () {
  ...
  */
  ...
  /*
    ...
    実測（実データの ja、データ行 1769・見出し 203、1280x800、Windows の Chromium）。
    重いのは条件を当てはめたあとの描き直しで、出す行が大きく入れ替わるとき（1字消して
    全行に戻るときなど）がいちばん重い。画面の外の行を描かないようにした（app.css の
    content-visibility）いまでも、検索語を消して全行に戻すと、待ちのあとの描き直しまでに
    約 65ms かかる（描かない指定を外すと約 160ms）。打鍵ごとに走らせると、この
    描き直しが打つ手に追いつかない。
