...
//
// 上流の CI の Python 3.12 に合わせてある。3.14 は切り出し方が変わり、先頭に
// 続く '.' を飛ばしてから最後の '.' を探す。そのため "..png" と "...png" は ""、
// "..PNG" も "" になり、3.12（".png"、".PNG"）と判定が割れる。手元の 3.14 で
// 上流を走らせると "..png" は置いてはいけないファイルになるが、CI と dwloc は
// 絵として扱う。3.14 は "a." にも "." を返すが、".png" ではないので判定は同じ。
