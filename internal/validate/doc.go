...
// cc01bfc（textures/）、912f519（credits.txt）、caa470b（状態語の native）に
// 追いついている。いま合わせてある check-translations.py は blob 3948566 で、
// 上流の dev（5e14be8）と main（4e8a2e8）のどちらもこの版。
//
// 翻訳者の Pull Request は、これまでどおりフォークから main へ出す（上流 130d57e。
// dev はメンテナーの作業をためる場所）。そのため Pull Request の CI は main の
// check-translations.py で走る。上流はこれまで dev をまとめて main へ入れてきた
// （56420df、#44。912f519 は #49、caa470b は #50）ので、dev が先に進んでも次の
// リリースで main に入る見込みで、ここでは dev の版に先に合わせる。
// dev だけが先に進んでいるあいだは、その差の分だけ main の CI と判定が割れる。
// いまは main と dev が同じなので、割れるのは写さなかった上流の不具合（下の
// 「元実装と意図的に変えたところ」）にあたる入力だけで、そこではここだけが
// 問題を報告する。逆に、上流の変更に追いつくまでは、上流が通す入力をここだけが
// 問題として報告する（caa470b に追いつく前の、状態語 native がそれ）。
//
// このパッケージが守りたいものは2つある。
//
...
// 上流 dev（5e14be8）の17ロケールに対しては、元実装と同じく問題0件になる。
package validate
